#' Set explanatory variables for Multi-stage LS
#' @S3method setx twosls
...
setx.twosls <- function(obj, fn=NULL, data=NULL, cond=FALSE, ...) {
  ...
  form2 <- parse.formula(form, "twosls")
  tt <- terms(form2)

  mod <- model.matrix(tt, data = d)
...
getPredictorTerms <- function (x, ...) {
  ...
  # @author Matt Owen
  extractFromList <- function (x, ...) {
    term.names <- unlist(Map(extractFromFormula, x))
    names(term.names) <- NULL
    term.names
  }
